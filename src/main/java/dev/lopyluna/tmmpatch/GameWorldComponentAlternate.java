...
public class GameWorldComponentAlternate {
    ...
    public void tickTriggerModifier(ServerWorld world) {
        ...
        if (modifier == GameModifier.BLACKOUT) {
            ...
            for (var player : players) player.networkHandler.sendPacket(new PlaySoundS2CPacket(Registries.SOUND_EVENT.getEntry(TMMSounds.AMBIENT_BLACKOUT), SoundCategory.PLAYERS,
                    player.getX(), player.getY(), player.getZ(), 100f, 1f, world.random.nextLong()));
